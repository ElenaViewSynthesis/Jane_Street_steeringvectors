...
if [ "$#" -ne 16 ]; then
    echo "activation_sandbox_entry.sh received an invalid argument count" >&2
    exit 64
fi
...
cpu_seconds=${13}
memory_bytes=${14}
output_limit_bytes=${15}
sandbox_profile=${16}

worker_path="$scripts_path/activation_worker.py"
